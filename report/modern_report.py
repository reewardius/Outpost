...
def generate_html_report(vulnerabilities, input_filename, additional_files=None):
    ...
    # Debug logging for dashboard values
    ...
    # Prepare display values for Dashboard
    ...
    # Additional debug to confirm display values
            ...
            for vuln in vulns:
                ...
                if vuln["extractors"]:
                    for extractor in vuln["extractors"]:
                        extractors_html += f'<span class="inline-block bg-gray-200 dark:bg-gray-600 text-gray-7 dark:text-gray-300 px-2 py-1 rounded text-sm">{html.escape(extractor)}</span>'
                else:
